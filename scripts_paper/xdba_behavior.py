...
if __name__ == "__main__":
    ...
    cfg.RESULTS_DIR = "/nfscc/alg23/xdba/b3"
    ...
    cfg.EXPERIMENTAL.USE_BHV = True
    cfg.EXPERIMENTAL.USE_BHV_PASSTHROUGH = True
    cfg.EXPERIMENTAL.BACKBONE_NOGRAD = False
    ...
    collection = [future, button, is_old, rtt, run, trial, run_trail]
    collection = [x.tolist() for x in collection]
    
    all_bhv = np.concatenate(collection)
    
    collection = [future, button, is_old, rtt, run, trial, run_trail, all_bhv]
    collection = [x.tolist() for x in collection]
    ...
    tune_config = {
        ...
        "EXPERIMENTAL.USE_BHV": tune.grid_search([True]),
        "EXPERIMENTAL.USE_BHV_PASSTHROUGH": tune.grid_search([True]),
        "EXPERIMENTAL.BEHV_ONLY": tune.grid_search([True]),
        "EXPERIMENTAL.BEHV_SELECTION": tune.grid_search(collection),
        "MODEL.COND.IN_DIM": tune.sample_from(lambda spec: int(len(spec.config['EXPERIMENTAL.BEHV_SELECTION']))),
    }
    name = f"bhv"
    ...
    tune_config = {
        "DATASET.SUBJECT_LIST": tune.grid_search([['subj01'], ['subj02'], ['subj03'], ['subj04'], ['subj05'], ['subj06'], ['subj07'], ['subj08']]),
        "EXPERIMENTAL.BLANK_IMAGE": tune.grid_search([False]),
        "EXPERIMENTAL.USE_BHV": tune.grid_search([False]),
        "EXPERIMENTAL.USE_BHV_PASSTHROUGH": tune.grid_search([False]),
        "EXPERIMENTAL.BEHV_ONLY": tune.grid_search([False]),
        # "EXPERIMENTAL.BEHV_SELECTION": tune.grid_search(collection),
        # "MODEL.COND.IN_DIM": tune.sample_from(lambda spec: int(len(spec.config['EXPERIMENTAL.BEHV_SELECTION']))),
    }
